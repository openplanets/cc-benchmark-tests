...
(function() {
  d3.parsets = function() {
    ...
    return d3.rebind(parsets, event, "on").value(1).width(1000).height(600);
  ...
  function defaultTooltip(d) {
    ...
    return path.join(" → ") + "<br>" + percent(count / d.count);
//   return path.join(" → ") + "<br>" + comma(count) + " (" + percent(count / d.count) + ")";
  }

  function defaultCategoryTooltip(d) {
    return d.name + "<br>" + percent(d.count / d.dimension.count);
